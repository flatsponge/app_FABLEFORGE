...
/**
 * Idempotent claim mutation for teaser image generation.
 * Returns teaser data only if successfully claimed (status was null, "queued", or "failed").
 * This prevents duplicate processing from scheduler retries.
 */
export const claimTeaserImageGeneration = internalMutation({
  args: { teaserId: v.id("onboardingTeasers") },
  returns: v.union(
    v.object({
      claimed: v.literal(true),
      prompt: v.string(),
      mascotStorageId: v.id("_storage"),
    }),
    v.object({
  ...
  handler: async (ctx, args) => {
    ...
    // Already complete - no need to regenerate
    if (teaser.teaserImageStatus === "complete" && teaser.teaserImageStorageId) {
      return { claimed: false as const, reason: "already_complete" as const };
    }

    // Already being generated by another process
    if (teaser.teaserImageStatus === "generating") {
      return { claimed: false as const, reason: "already_generating" as const };
    }

    // No mascot available - can't generate teaser image
    if (!teaser.mascotStorageId) {
      return { claimed: false as const, reason: "no_mascot" as const };
    }
    ...
    return {
      ...
      prompt: teaser.prompt,
      mascotStorageId: teaser.mascotStorageId,
    };
  },
});
