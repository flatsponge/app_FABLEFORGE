...
export default function ReadingScreen() {
  ...
  return (
    <View className="flex-1 bg-slate-900">
      ...
      <View className="absolute top-14 left-0 right-0 px-6 flex-row justify-between items-center z-50">
        <Pressable 
          onPress={handleClose}
      ...
      <View className="absolute bottom-0 left-0 right-0 bg-white rounded-t-[32px] px-6 pt-8 pb-10" style={{ minHeight: 380 }}>
        {!showCompletion ? (
          <>
            ...
            {isChildMode && (
              <View className="flex-row gap-4 h-24">
                <Pressable 
                  onPress={handleBack}
                  disabled={page === 0}
                  className={`flex-1 rounded-3xl items-center justify-center flex-row gap-2 ${
                    page === 0 ? 'bg-slate-100' : 'bg-amber-400'
                  }`}
                >
                ...
                <Pressable 
                  onPress={handleNext}
                  disabled={!canAdvance}
                  className={`flex-[2] rounded-3xl items-center justify-center flex-row gap-2 ${
                    !canAdvance ? 'bg-slate-200' : 'bg-emerald-400'
                  }`}
                >
                  {!canAdvance ? (
                    <>
            ...
            {isParentMode && (
              <View className="flex-row items-center justify-between gap-6">
                <Pressable 
                  onPress={handleBack}
                  disabled={page === 0}
                  className={`w-16 h-16 rounded-2xl items-center justify-center border-2 border-slate-100 ${
                    page === 0 ? 'opacity-0' : ''
                  }`}
                >
            ...
            <View className="flex-row justify-center gap-4 mb-6">
              <Pressable 
                ...
                className={`w-16 h-16 rounded-2xl items-center justify-center ${
                  rating === 'up' ? 'bg-green-500' : 'bg-slate-100'
                }`}
              >
              ...
              <Pressable 
                ...
                className={`w-16 h-16 rounded-2xl items-center justify-center ${
                  rating === 'down' ? 'bg-red-500' : 'bg-slate-100'
                }`}
              >
